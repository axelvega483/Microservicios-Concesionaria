...
public class VehiculoController {
    ...
    public ResponseEntity<?> obtenerVehiculo(@PathVariable Integer id) {
        try {
            ...
            if (vehiculo != null) {
                ...
                return new ResponseEntity<>("No se encontro vehiculo", HttpStatus.NOT_FOUND);
            }
        } catch (Exception e) {
            return new ResponseEntity<>("Error: " + e.getMessage(), HttpStatus.INTERNAL_SERVER_ERROR);
        }
    ...
    public ResponseEntity<?> subirImagenesVehiculo(@PathVariable Integer id,
        ...
        try {
            ...
            return new ResponseEntity<>("Error al subir imágenes: "+e.getMessage(),HttpStatus.INTERNAL_SERVER_ERROR);
        }
    }
    ...
    public ResponseEntity<?> verImagen(@PathVariable Integer vehiculoId, @PathVariable Integer imagenId) {
        try {
            ...
            if (response.getStatusCode().is2xxSuccessful()) {
                ...
                return new ResponseEntity<>("Error al obtener imagen",HttpStatus.INTERNAL_SERVER_ERROR);
            }

        } catch (Exception e) {
            return new ResponseEntity<>("Error interno al obtener imagen: " + e.getMessage(),HttpStatus.INTERNAL_SERVER_ERROR);
        }
    }
